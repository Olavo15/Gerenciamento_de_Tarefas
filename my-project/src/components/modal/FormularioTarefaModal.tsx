...
export function FormularioTarefaModal(props: Iprops){
    ...
    function verificacao(){
        if(formTarefa.titulo.length <= 0){
            setError('Verifique o titulo da tarefa!')
            return;
        }
        if(formTarefa.descricao.length <= 0 || formTarefa.descricao.length > 150){
            if(formTarefa.descricao.length > 150){
                setError('Descricao muito grande!')
            }
            setError('Verifique o Descricao da tarefa!')
            return;
        }
        if(formTarefa.id_projeto <= 0){
            setError('Erro servidor id projeto')
            return;
        }
        if(formTarefa.id_tabela_tarefa <= 0){
            setError('Erro servidor id tabela')
            return;
        }
        setError('')
    }
    ...
    function criarTarefa(){
        verificacao()
        api.post('/tarefa',formTarefa).then(response => console.log(response))
        props.closeModal()
    }
