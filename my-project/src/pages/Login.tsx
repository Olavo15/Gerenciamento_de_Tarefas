import { Envelope, Password } from "@phosphor-icons/react";
import { ChangeEvent, FormEvent, useEffect, useState } from "react";
import api from "../service/api";

export default function Login(){

    const [loginForm, setLoginForm] = useState({
        email: '',
        senha: ''
    })

    const [error, setError] = useState('')


    function verificaCaoFormulario(){
        if(loginForm.email.length < 4){
            setError('Tem certeza que é só isso ?')
            return false
        } 
        if(loginForm.senha.length < 4){
            setError('Tem certeza que é só isso ?')
            return false
        } 
        setError('')
        return true
    }

    async function login(e: FormEvent<HTMLFormElement>){
        e.preventDefault();
        const isFormValid =  verificaCaoFormulario();
        if (isFormValid) {
            api.post('/login', loginForm)
                .then(response => {
                    console.log('Usuário cadastrado com sucesso:', response.data);
                    setLoginForm({
                        senha: '',
                        email: '',
                    });
                    setError('');
                    localStorage.setItem('userData', JSON.stringify(response.data))
                    window.location.href = '/'
                })
                .catch(error => {
                    console.error('Erro ao cadastrar usuário:', error);
                    console.log(error.response.data)
                    setError(error.response.data.error)
                });
        }
    }


    return(
        <div className="flex items-center justify-center w-screen h-screen" >
            <form className="flex items-center justify-center px-8 py-10 border border-[#c6c6c6] bg-zinc-50 shadow-lg rounded-2xl w-fit flex-col gap-3" onSubmit={(e: FormEvent<HTMLFormElement>) => { e.preventDefault(); login(e); }}>
                <img src="data:image/jpeg;base64," alt="GC"
                 className=" mb-1 w-28 h-28 rounded-md"/>
                <div className="flex items-center  rounded-md border border-[#2d2d2d] px-3 py-1">
                    <input value={loginForm.email} onChange={(e: ChangeEvent<HTMLInputElement>) => setLoginForm({ ...loginForm, email: e.target.value })} type="text" className="flex-1 bg-transparent" placeholder="Insira seu E-mail" />
                    <Envelope/>
                </div>
                <div className="flex items-center  rounded-md border border-[#2d2d2d] px-3 py-1">                             
                    <input value={loginForm.senha} onChange={(e: ChangeEvent<HTMLInputElement>) => setLoginForm({ ...loginForm, senha: e.target.value })} type="password" className="flex-1 bg-transparent" placeholder="Insira sua senha" />
                    <Password/>
                </div>
                <p className="text-red-500 font-semibold">
                    {error.length > 4 ? error : null}
                </p>
                <div className="w-full px-2 py-[7px] text-base text-center text-white bg-[#116291] rounded-md">
                    <button type="submit">Entrar</button>
                </div>
                <div className="flex items-center justify-between w-full ">
                    <a className="border-b border-gray-500" href="/cadastro">
                        Registre-se
                    </a>
                </div>
            </form>
        </div>
    )
}
